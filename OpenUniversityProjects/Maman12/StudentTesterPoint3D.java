...
public class StudentTesterPoint3D {
    ...
    private static void testPoint3D() {
        ...
        if (p2.isUnder(p1)) {
            System.out.println("\tisUnder is not correct with: p2.isUnder(p1)");
        }
        if (!p1.isUnder(p2)) {
        ...
        if (p1.isRight(p2)) {
            System.out.println("\tisRight is not correct with: p1.isRight(p2)");
        }
        if (!p2.isRight(p1)) {
        ...
        if (p1.isInFrontOf(p2)) {
            System.out.println("\tisInFrontOf is not correct with: p1.isInFrontOf(p2)");
        }
